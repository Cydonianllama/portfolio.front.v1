...
type LayoutModuleProps = {
  layoutFor?: 'table' | 'list' | 'compactmodule',
...
export function LayoutScreen({ children, layoutFor = 'table', domConfig }: PropsWithChildren<LayoutModuleProps>){
  ...
  return <>
    <div className={`${cn("relative w-full h-full bg-background", className)}  ${layoutFor == 'table' && 'px-30'} ${layoutFor == 'list' && 'px-90'} ${layoutFor == 'compactmodule' && 'px-0'}`}>
      {children}
    </div>
  </>
}